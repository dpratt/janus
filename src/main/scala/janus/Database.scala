...
trait Session extends CloseableResource {
  ...
  def withPreparedStatement[T](query: String, withAutoGeneratedKeys: Boolean = false)(f: PreparedStatement => T): T = {
    withResource(prepareStatement(query, withAutoGeneratedKeys))(f)
  }
   ...
   */
  def prepareStatement(query: String, returnAutoGeneratedKeys: Boolean = false): PreparedStatement
...
class JdbcSession(val database: JdbcDatabase) extends Session {
  ...
  def failureHandler(e: Throwable) {
    log.error("Closing JdbcSession due to exception.")
    close()
  }
  ...
  def prepareStatement(query: String, returnAutoGeneratedKeys: Boolean = false): PreparedStatement  = {
    ...
    if (returnAutoGeneratedKeys) {
      ...
      new JdbcPreparedStatement(conn.prepareStatement(query, java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE,
  ...
  def statement(): Statement = {
    ...
    new JdbcStatement(conn.createStatement(java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE,
      java.sql.ResultSet.CONCUR_READ_ONLY))
  }
}
